...
class Engine:
    ...
    def eraseHexagonsbyPlayer(self):
        list = [self.findHexagonsToErase(self.setsetPy, self.setsetPx),
                self.findHexagonsToErase(self.setPy, self.setPx)]
        for l in list:
            for j in range(0, 3):
        ...
        for i in range(len(self.listOfNeighborsOdd)):
            ...
            while True:
                ...
                if helperCoords[1] + l[i][1] >= 0 and \
                    ...
                    if self.board.hexagonslist[helperCoords[0] + l[i][0]][helperCoords[1] + l[i][1]].type == \
                            self.board.hexagonslist[y][x].type \
                            and not self.board.hexagonslist[helperCoords[0] + l[i][0]][
                        helperCoords[1] + l[i][1]].isEmpty:
    ...
    def swap(self):
        ...
        self.board.hexagonslist[self.setsetPy][self.setsetPx].type = self.board.hexagonslist[self.setPy][
            self.setPx].type
        ...
        self.prepareMap()
        if self.isPlayer1: self.isPlayer1 = False
        else: self.isPlayer1 = True
        ...
        if self.setPx % 2 == 1:
            while True:
                ...
                if self.setPx + self.listOfNeighborsOdd[self.neighborindex][1] >= 0 and \
                        self.setPx + self.listOfNeighborsOdd[self.neighborindex][1] < len(
                    self.board.hexagonslist[self.setPx]) \
            ...
            while True:
                ...
                if self.setPx + self.listOfNeighborsPairWise[self.neighborindex][1] >= 0 and \
                        self.setPx + self.listOfNeighborsPairWise[self.neighborindex][1] < len(
                    self.board.hexagonslist[self.setPx]) \
    ...
    def prepareMap(self):
        self.eraseHexagons()
        self.isSettling = self.gravity()
        self.eraseHexagons()

    def game(self):
        self.gameScore = 0
        if self.isSettling: self.prepareMap()
        else:   self.movePlayer()
        if self.isSettling: return True, self.isPlayer1, self.gameScore
        else: return False, self.isPlayer1, self.gameScore
